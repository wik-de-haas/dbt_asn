...
# Change to the bigquery-app directory (the dbt project root)
cd "$(dirname "$0")/../../" || { echo "Failed to cd to bigquery-app directory"; exit 1; }
...
if [[ "$OS_TYPE" == "wsl" ]]; then
  ...
  TEMP_PROJECT_PATH="/tmp/dbt-bigquery-setup-$$"
  ORIGINAL_DIR="$PWD"
...
# BigQuery service account sanity check
if grep -q 'method: service-account' profiles.yml 2>/dev/null; then
  if [[ -z "${GOOGLE_APPLICATION_CREDENTIALS:-}" ]] || [[ ! -f "${GOOGLE_APPLICATION_CREDENTIALS:-/nonexistent}" ]]; then
    echo "⚠️  GOOGLE_APPLICATION_CREDENTIALS is not set or file not found."
    echo "    If you are running in Docker, mount your key and set the env var in docker-compose.yml."
    echo "    If running locally, export the env var to point to your key file."
  else
...
echo "To use BigQuery service account in Docker, ensure docker-compose mounts /secrets/gcp-sa.json and sets GOOGLE_APPLICATION_CREDENTIALS=/secrets/gcp-sa.json"
echo "For local OAuth, run: gcloud auth application-default login and set method: oauth in profiles.yml"
