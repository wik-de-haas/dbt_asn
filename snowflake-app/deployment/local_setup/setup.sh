...
# Change to the snowflake-app directory (the dbt project root)
cd "$(dirname "$0")/../../" || { echo "Failed to cd to snowflake-app directory"; exit 1; }
...
if [[ "$OS_TYPE" == "wsl" ]]; then
  ...
  TEMP_PROJECT_PATH="/tmp/dbt-snowflake-setup-$$"
  ORIGINAL_DIR="$PWD"
...
if command -v dbt &> /dev/null; then
  # Disable set -e temporarily to safely capture the version output
  set +e
...
# Snowflake connection sanity check
if grep -q 'type: *snowflake' profiles.yml 2>/dev/null; then
  if [[ -n "${SNOWFLAKE_ACCOUNT:-}" ]] && [[ -n "${SNOWFLAKE_USER:-}" ]]; then
    if [[ -n "${SNOWFLAKE_PASSWORD:-}" ]]; then
      echo "✓ Snowflake configuration detected (username + password)."
      echo "  Account: ${SNOWFLAKE_ACCOUNT}"
      echo "  User:    ${SNOWFLAKE_USER}"
    elif [[ -n "${SNOWFLAKE_PRIVATE_KEY_PATH:-}" ]] && [[ -f "${SNOWFLAKE_PRIVATE_KEY_PATH}" ]]; then
      echo "✓ Snowflake configuration detected (key pair)."
      echo "  Account:          ${SNOWFLAKE_ACCOUNT}"
      echo "  User:             ${SNOWFLAKE_USER}"
      echo "  Private key path: ${SNOWFLAKE_PRIVATE_KEY_PATH}"
    else
      echo "⚠️  Snowflake user/account set but no auth method found."
      echo "    Option A (dev):  set SNOWFLAKE_PASSWORD"
      echo "    Option B (prod): set SNOWFLAKE_PRIVATE_KEY_PATH pointing to your .p8 key file"
    fi
  else
    echo "⚠️  Snowflake connection not configured."
    echo "    Required for all targets: SNOWFLAKE_ACCOUNT, SNOWFLAKE_USER"
    echo "    Auth option A (dev/acc):  also set SNOWFLAKE_PASSWORD"
    echo "    Auth option B (prod):     also set SNOWFLAKE_PRIVATE_KEY_PATH"
  fi
fi
...
if [[ "$OS_TYPE" == "wsl" ]]; then
  echo "    source /tmp/venv-${PROJECT_NAME}/bin/activate"
else
  echo "    source .venv/bin/activate"
fi
echo "For Docker/CI: set SNOWFLAKE_ACCOUNT, SNOWFLAKE_USER, and either SNOWFLAKE_PASSWORD (dev) or SNOWFLAKE_PRIVATE_KEY_PATH (prod)"
echo "Ensure profiles.yml output is type: snowflake with matching account/user/auth settings"
