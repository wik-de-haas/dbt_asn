...
# Change to the databricks-app directory (the dbt project root)
cd "$(dirname "$0")/../../" || { echo "Failed to cd to databricks-app directory"; exit 1; }
...
if [[ "$OS_TYPE" == "wsl" ]]; then
  ...
  TEMP_PROJECT_PATH="/tmp/dbt-databricks-setup-$$"
  ORIGINAL_DIR="$PWD"
...
if command -v dbt &> /dev/null; then
  # dbt-databricks can exit non-zero due to thrift/pydantic warnings, so disable set -e temporarily
  set +e
...
# Databricks connection sanity check
if grep -q 'type: *databricks' profiles.yml 2>/dev/null; then
  if { [[ -n "${DATABRICKS_CONFIG_PROFILE:-}" ]] && [[ -f "${DATABRICKS_CONFIG_FILE:-$HOME/.databrickscfg}" ]]; } || \
     { [[ -n "${DATABRICKS_HOST:-}" ]] && [[ -n "${DATABRICKS_HTTP_PATH:-}" ]] && [[ -n "${DATABRICKS_TOKEN:-}" ]]; }; then
    echo "✓ Databricks configuration detected."
    if [[ -n "${DATABRICKS_CONFIG_PROFILE:-}" ]]; then
      echo "  Using Databricks CLI profile: ${DATABRICKS_CONFIG_PROFILE}"
    else
      echo "  Using env vars: DATABRICKS_HOST / DATABRICKS_HTTP_PATH / DATABRICKS_TOKEN"
    fi
  else
    echo "⚠️  Databricks connection not configured."
    echo "    Option A: set DATABRICKS_HOST, DATABRICKS_HTTP_PATH, DATABRICKS_TOKEN."
    echo "    Option B: set DATABRICKS_CONFIG_PROFILE and ensure ~/.databrickscfg is available (mount it in Docker if needed)."
  fi
fi
...
echo "For Docker: set DATABRICKS_HOST, DATABRICKS_HTTP_PATH, DATABRICKS_TOKEN or mount ~/.databrickscfg and set DATABRICKS_CONFIG_PROFILE"
echo "Ensure profiles.yml output is type: databricks with matching host/http_path/token or profile settings"
